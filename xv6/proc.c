...
uint va2pa(uint virtual_addr) {
  ...
  pde = &pgdir[PDX(virtual_addr)];

  if(*pde & PTE_P) {
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
    pte = &pgtab[PTX(virtual_addr)];
    return PTE_ADDR(*pte) | (virtual_addr & 0xFFF);
  }
  else return -1;
}
  ...
  for(int i=0; i<NPDENTRIES; i++) {
    if(pgdir[i] & PTE_P) {
      ...
      for(int j=0; j<NPTENTRIES; j++) {
        int va = (i << PDXSHIFT) | (j << PTXSHIFT);
        if(va < KERNBASE && (pgtab[j] & PTE_P)) pages++;
      }
    }
  }
  return pages;
