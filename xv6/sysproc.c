...
int sys_spawn(void) {
  ...
  if(argptr(1, (void*)&pids, n*sizeof(int)) < 0) return -1;
  return spawn(n, pids);
}
...
uint sys_va2pa(void) {
  int virtual_addr;
  if(argint(0, &virtual_addr)<0) return  -1;
  return va2pa((uint)virtual_addr);
}

int sys_get_pgtb_size() {
  return get_pgtb_size();
}

int sys_get_usr_pgtb_size() {
  return get_usr_pgtb_size();
}

int sys_get_kernel_pgtb_size() {
